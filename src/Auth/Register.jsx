...
import { registerInput } from "../Constant/websiteContent";
import { fetchRequest, getCookie } from "../Constant/ReqHelper";
    ...
    await fetchRequest(
      ...
      .then((res) => {
        ...
        if (res.code === 201) {
          ...
          setSuccessMsg(
            "Account Successfully, You are Redirecting to login Page, Please enter Your emailId and Password to next login page And verify Your Account"
          );
        }
      })
      .catch((err) => {
        ...
        err.response.data.code === 409
          ? setErrorMsg("Email Id is already exist in our database")
          : err.code === "ERR_BAD_REQUEST" &&
            setErrorMsg(err?.response?.data?.message?.message);
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  return (
    ...
    >
      ...
      <p className="mb tac">
        ...
        <Link to={"/login"} className="t-primary t-bold">
          Login
        </Link>
      </p>
      <hr />
      <form onSubmit={handleSubmit} method="POST" id="register-form">
        ...
        {errorMsg !== "" && <p className="mt t-danger">{errorMsg}</p>}
        {successMsg !== "" && <p className="mt t-success">{successMsg}</p>}
      </form>
    </div>
  );
};
